...
class WebsocketOutput {
  ...
  write(data) {
    ...
    if (this.leftBuffer.length >= this.bufferLength) {
      this.flush();
    }
  }
  ...
  flush() {
    ...
    const f32 = this.a2b(this.leftBuffer, this.rightBuffer);
    this.leftBuffer = [];
    this.rightBuffer = [];

    this.ws.send(f32.buffer);
  }
...
class ShapeFilter {
  ...
  updateShapeFromVideo() {
    ...
    const meanScalar = cv.mean(pre); // grayscale이라면 meanScalar[0]에 평균 밝기 들어있어
    const meanBrightness = meanScalar[0];
    const treshBin =
      meanBrightness > 127 ? cv.THRESH_BINARY_INV : cv.THRESH_BINARY;
